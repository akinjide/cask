    ...
    db.serialize(() => {
      db.all('SELECT * FROM permissions', (err, rows) => {
        if (!err && (rows && rows.length == 0)) {
          ...
          db.run('INSERT INTO permissions (p_user, p_group, p_other, entity_id, owner_id, group_id, last_modified) VALUES ($pUser, $pGroup, $pOther, $entityID, $ownerID, $groupID, $lastModified);', {
            $pUser: 7,
            $pGroup: 7,
            $pOther: 0,
            $entityID: 2,
